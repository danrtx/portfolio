...
function Toast({ message, type, onClose }: { message: string; type: 'success' | 'error'; onClose: () => void }) {
  ...
  return (
    ...
    >
      ...
      <button onClick={onClose} aria-label="Close message" style={{ background: 'none', border: 'none', color: 'rgba(240,244,255,0.4)', cursor: 'none', fontSize: '1rem' }}>×</button>
    </motion.div>
  );
}
...
export function ContactSection() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        showToast(lang === 'es' ? '¡Mensaje enviado! Te respondo en menos de 5h ✓' : "Message sent! I'll reply within 5h ✓", 'success');
      } else {
  ...
  const channels = [
    {
      icon: <svg viewBox="0 0 24 24" fill="currentColor" width="18" height="18" role="img" aria-hidden="true" focusable="false"><path d="M20 4H4c-1.1 0-2 .9-2 2v12c0 1.1.9 2 2 2h16c1.1 0 2-.9 2-2V6c0-1.1-.9-2-2-2zm0 4l-8 5-8-5V6l8 5 8-5v2z" /></svg>,
      label: 'Email', handle: 'danilomntzm@gmail.com', href: 'mailto:danilomntzm@gmail.com', accent: '#4F8EF7',
    },
    {
      icon: <svg viewBox="0 0 24 24" fill="currentColor" width="18" height="18" role="img" aria-hidden="true" focusable="false"><path d="M16 8a6 6 0 016 6v7h-4v-7a2 2 0 00-2-2 2 2 0 00-2 2v7h-4v-7a6 6 0 016-6zM2 9h4v12H2z M4 6a2 2 0 100-4 2 2 0 000 4z" /></svg>,
    ...
    {
      icon: <svg viewBox="0 0 24 24" fill="currentColor" width="18" height="18" role="img" aria-hidden="true" focusable="false"><path d="M12 2C6.477 2 2 6.477 2 12c0 4.42 2.865 8.167 6.839 9.49.5.092.682-.217.682-.482 0-.237-.008-.866-.013-1.7-2.782.604-3.369-1.34-3.369-1.34-.454-1.156-1.11-1.463-1.11-1.463-.908-.62.069-.608.069-.608 1.003.07 1.531 1.03 1.531 1.03.892 1.529 2.341 1.087 2.91.832.092-.647.35-1.088.636-1.338-2.22-.253-4.555-1.11-4.555-4.943 0-1.091.39-1.984 1.029-2.683-.103-.253-.446-1.27.098-2.647 0 0 .84-.269 2.75 1.025A9.578 9.578 0 0112 6.836c.85.004 1.705.114 2.504.336 1.909-1.294 2.747-1.025 2.747-1.025.546 1.377.202 2.394.1 2.647.64.699 1.028 1.592 1.028 2.683 0 3.842-2.339 4.687-4.566 4.935.359.309.678.919.678 1.852 0 1.336-.012 2.415-.012 2.743 0 .267.18.578.688.48C19.138 20.163 22 16.418 22 12c0-5.523-4.477-10-10-10z" /></svg>,
            ...
            <div className="flex gap-3">
              {socialLinks.map((sl) => (
                <motion.a key={sl.name} href={sl.href} target="_blank" rel="noopener noreferrer"
                  ...
                  aria-label={
                    sl.name === 'Email' ? 'Send email to danilomntzm@gmail.com'
                    : sl.name === 'LinkedIn' ? 'Visit LinkedIn profile'
